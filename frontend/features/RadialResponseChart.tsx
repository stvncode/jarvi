...
export function RadialResponseChart() {
    ...
    return (
      <Card className="flex flex-col">
        <CardHeader className="items-center pb-0">
          <CardTitle>Taux de réponse par canal</CardTitle>
  ...
  const TrendingIcon = globalChange >= 0 ? TrendingUp : TrendingDown
      ...
      <CardContent className="flex-1 pb-0">
        ...
        >
          <RadialBarChart data={chartData} innerRadius={30} outerRadius={110}>
                  ...
                  return (
                    <div className="bg-background border rounded-lg shadow-lg p-3">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Taux de réponse: {data.response_rate}%
                      </p>
                      <p className="text-sm text-muted-foreground">
                        {data.total_replied}/{data.total_sent} messages
                      </p>
                    </div>
                  )
                }
            ...
            />
            <RadialBar dataKey="response_rate" background />
          </RadialBarChart>
        </ChartContainer>
      </CardContent>
      <CardFooter className="flex-col gap-2 text-sm">
        {stats.comparison && (
          <div className="flex items-center gap-2 leading-none font-medium">
            Évolution de {Math.abs(globalChange).toFixed(1)}% cette période <TrendingIcon className="h-4 w-4" />
          </div>
        )}
        <div className="text-muted-foreground leading-none">
          Taux de réponse global: {stats.overall_response_rate}%
        </div>
      </CardFooter>
    </Card>
  )
